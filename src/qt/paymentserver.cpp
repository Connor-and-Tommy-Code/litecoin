...
const QString BITCOIN_IPC_PREFIX("TCC:");
#ifdef ENABLE_BIP70
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/TCC-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/TCC-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/TCC-paymentrequest";
#endif
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start TCC: click-to-pay handler"));
        }
        else {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("TCC://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'TCC://' is not a valid URI. Use 'TCC:' instead."),
    ...
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid TCC address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
void PaymentServer::LoadRootCAs(X509_STORE* _store)
{
    ...
    if (certFile != "-system-") {
        ...
        QSslSocket::setDefauTCCaCertificates(certList);
    } else
...
void PaymentServer::fetchPaymentACK(WalletModel* walletModel, const SendCoinsRecipient& recipient, QByteArray transaction)
{
    ...
    if (walletModel->wallet().getKeyFromPool(false /* internal */, newKey)) {
        ...
        const OutputType change_type = walletModel->wallet().getDefauTCChangeType() != OutputType::CHANGE_AUTO ? walletModel->wallet().getDefauTCChangeType() : walletModel->wallet().getDefaultAddressType();
